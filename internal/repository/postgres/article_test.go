	...
	tests := []struct {
		...
		// },
		{
			...
			mock: func() {
				...
				sqlMock.ExpectQuery(regexp.QuoteMeta(`SELECT`)).WillReturnError(errors.New("failed to get article details"))
			},
		},
	}
	for _, tt := range tests {
...
func Test_repository_UpdateArticle(t *testing.T) {
	...
	tests := []struct {
		...
		// 		sqlMock.ExpectQuery(regexp.QuoteMeta(`SELECT a.id, a.title, a.slug, a.html_content, c.id, c.title , c.slug, a.created_at, a.updated_at FROM cms_article a JOIN cms_category c ON a.category_id = c.id WHERE a.id = $1`)).WithArgs(1).WillReturnRows(rows)
